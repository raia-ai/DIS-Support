...
short_description: "A chapter from the DIS Business System Manual about APPENDIX E."
long_description: "This document is a chapter from the DIS Business System Manual, focusing on APPENDIX E."
---

## FORMS ALIGNMENT IBM 5224 PRINTER
...
- Let them print! If you need to make another minor adjustment, take the printer off-line temporarily. It will resume exactly where it left off when you put it back on-line. If you have a forms jam or other semi-

## APPENDIX E: FORMS ALIGNMENT IBM 5224 PRINTER

catastrophe, see the section TROUBLESHOOTING.
...
7     X12700     Printer message (use Opt 7)
      P9
__    X51088     Printer stopped (use Opt 10)
__    X51088     Printer stopped (use Opt 10)
__    X51088     Printer stopped (use Opt 10)
__    X51088     Printer stopped (use Opt 10)
__    X51088     Printer stopped (use Opt 10)
...
The first form will be used for alignment. Align the left perforation with the 0 (zero) on the ruled silver bar.

**Vertical Alignment**
Place checks in tractor so that the top of the first check is even with the top of the left tractor. The center of the second hole will be lined up with the top of the right tractor. If necessary, roll the checks to make minor adjustments. Use the blue wheel at the far left to roll.
...
If checks are aligned as described they should only need minor adjustment.

If the alignment needs more than minor adjustment, press the Stop button on the printer. See the section titled TROUBLESHOOTING.

## RECEIVABLES STATEMENTS & PAST DUE STATEMENTS

Regular and interim/past due statements are printed on the same forms, so the alignment is the same.
...
Before you answer the message, put the statements in the tractor feed so that they are lined up like this:

**Horizontal Alignment**

The left perforation should be lined up with the 0 (zero) on the ruled bar.

**Vertical Alignment**

The first form will be used for alignment. Draw a line on the first statement 1 3/4 inches from the top. Place the first statement so this line is even with the top of the left tractor. Use the roll wheel at the left to make slight adjustments if necessary.

When statements are loaded, answer the message with aG1. The forms will advance several lines and the first line of the address on the first statement will print. You will receive the alignment message.

If statements are aligned as described they should only need minor adjustment.


To check alignment, use the blue wheel on the left. Roll the right part of the wheel until the black pointer is on 0. Using the left part of the wheel, roll the paper up until you can see the printing. Note any necessary adjustments. Using the left side of the wheel again, roll the paper until the pointer is back on 0. From this position roll the paper either up or down as needed. When you are satisfied with the alignment, answer the message with a G. The rest of the statements will print.

If the alignment is not correct, press the Stop button on the printer and see the next section titled TROUBLESHOOTING.
...
If the alignment is slightly off:
...
If the alignment is completely wrong, you can start the job over.

- Display the print spool by typing `DP` at the bottom of any menu.
