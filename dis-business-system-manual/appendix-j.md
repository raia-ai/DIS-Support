...
short_description: "A chapter from the DIS Business System Manual about APPENDIX J."
long_description: "This document is a chapter from the DIS Business System Manual, focusing on APPENDIX J."
---

## FORMS ALIGNMENT OKIDATA 393 PRINTER
...
## APPENDIX J: FORMS ALIGNMENT OKIDATA 393 PRINTER
...
Watch the printer output screen to know when your special forms are to be printed. At a command line, type: D P [Enter].
...
Opt Output Status
P1
? X12700 Printer message (use Opt 7)
P9
...
| **Possible choices for replying to message**  G | Begin processing the current file after loading the form type. 
...
| I | Ignore the request to load the form type. Print the file on the current formtype (same as option 0 on System/36). | | More... |
| Type reply below, then press Enter.  **Reply** | G 
| F3=Exit | F6=Print | F9=Display message details | F12=Cancel |
| F21=Select assistance level  |
...
When running payables checks, do not change forms until you are prompted to do so. The check register will print first. If you have changed forms to checks, the register will print on the checks. When the register finishes printing, you will receive a message telling you to load checks.

Before you answer the message align the checks in the printer as follows:

**Horizontal Alignment**

The first form will be used for alignment. Align the perforation on the right tear-off strip with number 74 on the paper bail.

**Vertical Alignment**

Align checks so that the paper tear-off bar is between "Phone" and "(206)". These words are printed on the left tear-off strip on forms from Moore Printing Co. Otherwise, align checks so that the paper tear-off bar is even with the top of the 3rd hole from the bottom of the check stub.
...
If checks are aligned as described they should only need minor adjustment. Make minor adjustments using the roll wheel at the left.

If more than a minor adjustment is needed, press the Stop button on the printer. See the section titled TROUBLESHOOTING.

When checks have printed, change the printer parameters back to the correct settings for standard forms.

## RECEIVABLES STATEMENTS & PAST DUE STATEMENTS

Regular and interim/past due statements are printed on the same forms so the alignment is the same for both.
...
Align the perforation on the left tear-off strip even with the number one on the paper bail.

### Vertical Alignment

Align checks so that the paper tear-off bar is between "(206)" and "733". This phone number is printed on the left tear-off strip on forms from Moore Printing Co. Otherwise, align checks so that the paper tear-off bar is even with the top of the 3rd hole from the bottom of the check stub.


When statements are loaded, answer the message with a G. The forms will advance several lines and the first line of the address on the first statement will print. You will receive the alignment message. If statements are aligned as described they should only need minor adjustment. Make minor adjustments using the roll wheel.

When you are satisfied with the alignment, answer the message with a G. The rest of the statements will print. If more than a minor adjustment is needed, press the Stop button on the printer. See the section titled TROUBLESHOOTING.
...
## APPENDIX J: FORMS ALIGNMENT OKIDATA 393 PRINTER 15
