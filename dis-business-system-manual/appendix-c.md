...
short_description: "A chapter from the DIS Business System Manual about APPENDIX C."
long_description: "This document is a chapter from the DIS Business System Manual, focusing on APPENDIX C."
---

## FORMS ALIGNMENT 4214 PRINTER
...
## APPENDIX C: FORMS ALIGNMENT IBM 4214 PRINTER
...
Although there seem to be dozens of messages the first time you try to


align forms, there are really only 2 different messages:
...
When you run a job that requires a change of forms, look at the AS/400's "Work with Printer Output" screen. At any DIS menu, type: D P [Enter]. The screen is illustrated below. In the example, messages for Past Due Statements (X11-11) is illustrated; however, you will encounter the same messages in Past Due Statements, Print Payables Checks, printing Letters (new A/R) and Labels.
...
Cause . . . . . :   The file on output queue PRT01 in library QUSRSYS requires form type 'CHKS' to be loaded on device PRT01. The form type for the file was all blanks when '' appears as the form type.
...
- **C =** To cancel processing, type a C. **DO NOT USE THIS OPTION.** You cannot re-run Past Due Statements or Print Payables Checks or Payroll unless you restore a backup, which means you may have to repeat a lot of work.
...
Possible choices for replying to message . . . . . . . . . . . . . . More...
  G -- Begin processing the current file after loading the form type.
...
When running payables checks, do not change forms until you are prompted to do so. The check register will print first. If you have changed forms to checks, the register will print on the checks.

When the register finishes printing, you will be prompted to load the checks.
Before you answer the message put the checks in the tractor feed so that they are lined up like this:

**Horizontal Alignment**

From the front of the printer, the left edge of the check should be even with the left end of the red strip on the metal alignment bar.

**Vertical Alignment**

The first check and check stub are used for alignment. Draw a line on the check stub 2 1/2 inches below the perforation between the check and the stub. Place checks so that the metal bar is right on this line. Use the up and down arrows on the front panel of the printer to make minor adjustments. Never use the line feed key to make adjustments.

When the checks are loaded, answer the message with a G. The check number of the first check will print. You will receive the align forms message.

If checks are aligned as described, they should only need minor adjustment. Make minor adjustments using the up and down arrow keys.

If more than a minor adjustment is needed, press the Stop button on the printer. See the section titled TROUBLESHOOTING.

## RECEIVABLES STATEMENTS & PAST DUE STATEMENTS

Regular and interim/part due statements are printed on the same forms so the alignment is the same.

When statements are ready to be printed the system will prompt you to load the statements. The balance forward statements will print first, followed by the open item. You will be prompted to align forms for both types of statements. When open item statements are ready to print, they will be aligned correctly and all you will have to do is answer the messages.
...
Before you answer the message put the statements in the tractor feed so they are lined up like this:

**Horizontal Alignment**

The left perforation should be even with the 0 mark on the scale of 0 to 13.

**Vertical Alignment**

The first form will be used for alignment. Draw a line on the first statement 7 1/2 inches below the top of the statement. Place the statements so that this line is right on the metal bar. Use the up and down arrow keys to make slight adjustments if necessary.


When statements are loaded, answer the message with a G. The forms will advance several lines and the first line of the address on the first statement will print. You will receive the alignment message.

If statements are aligned as described they should only need minor adjustment. Make minor adjustments using the roll wheel at the left.

When you are satisfied with the alignment, answer the message with a G. The rest of the statements will print.
...
Move the left tractor as far right as it will go. The left edge of the invoice should be even with the left end of the red strip on the metal alignment bar.

Vertical Alignment

Load the invoices so the perforation between invoices is just slightly above the plastic cover when the cover is closed. Use the up and down arrow keys to make slight adjustments if necessary. Do not use the linefeed key.
...
_If the alignment is slightly off:_
Decide which way the form is off. Use the roll wheel at the far left to make the necessary adjustment.

- Press Start on the printer.

_If the alignment is completely wrong, you can start the job over._

- Display the print spool by typing D P at the bottom of any menu.

- Select option 3=Hold. Put this print job (and any other print jobs) on hold.

If you are printing checks the print spool will display CHKS in the column for FORM. Statements will display STAT. Change the forms listed for the print job by taking option 5 and then option 2=Change.

- Change the forms to XXXX or anything other than CHKS or STAT. Because all printer buffers are set differently, the current page spooled to the printer may or may not be the actual page printing. We recommend starting back 5 to 10 pages when choosing a page number to start to reprint. Change the next page to print [Enter].


- Take the option to release the held print job.
- The job will start over and you will receive the same messages.