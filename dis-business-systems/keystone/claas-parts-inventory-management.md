...
long_description: "This document provides information about CLAAS Parts Inventory Management from the DIS Keystone system."
---
...
4.  Type the Dealer Number supplied by CLAAS in the **Inventory Dealer Number** field. This is for the current inventory download. (Example: `AL34892`)
5.  Type the Warehouse Group Code provided by CLAAS into the **Warehouse Group Code** field. There is only one code per file set. Once it has been entered in one division, the number supplied will be verified for a match when new divisions are configured in the same file set. (Example: `1100_US_0000022019`)
6.  Type the Warehouse Code provided by CLAAS into the **Warehouse Code** field. Each individual division will have its own unique warehouse code. (Example: `0000022019`)
...
You can view when part inventory files were sent to your dealership from CLAAS and whether or not the submission was successful.
...
The Work with Suggested Transfers option allows you to view, print, change, delete, or complete an outgoing transfer. You can also view the parts on an incoming transfer and change the quantity if desired.

Transfers created by the system from the Order Export file will start with `CLT`, then the division code, then a four-digit number.
**For Example:** `CLTA0017`

### Outgoing Transfers

1.  Select **Work with Suggested Transfers** (`Manufacturers | CLAAS | Work with Suggested Transfers`). The Manufacturer Suggested Transfers screen opens displaying suggested transfers for select manufacturers.
2.  Select the transfer from the table that you would like to work with and do one of the following:
    *   **Display or Change**: Click the **Display** or **Change** button. A screen opens listing the parts on the transfer. If you want to change the quantity of parts to be transferred, click on the current quantity, type in the correct quantity, and click **Enter**.
    *   **Transfer**: Click the **Transfer** button to process the suggested transfer. The "Confirm processing of Transfer Parts" screen opens. Verify this is the transfer that you want to complete and click the **OK** button.
    *   **Delete**: Click the **Delete** button to remove the transfer from the system. The "Confirm Deletion of Parts Transfer" screen opens. Click the **OK** button to confirm the deletion. If you press Enter on your keyboard, the deletion process will be cancelled.
    *   **Print**: Click the **Print** button, then click **OK** on the screen that opens to print the suggested transfer.
...
3.  Enter the **Upload Date** that you want to consider when generating the report.
...
*   The CLAAS order description is included for reference as CLAAS breaks down their order types differently than Keystone.

```
A&R EQUIPMENT SALES                                        CLAAS Suggested Orders                           Date 2/14/19
Division      A                                              Order Details                              Time 11:32:29

Requested Date: 1/30/19

Orders Created:
Order #                      Lines Order Code CLAAS Order Desc
---------------              ----- ---------- --------------------
CLAA0047                         1 S          Regular - 1 day Shipping
CLAA0048                         1 S          Regular - Economy Shipping
CLAA0049                         1 S          Emergency
CLAA0050                         3 S          Weekly
CLAA0051                         1 S          Weekly

Items Not Ordered:
Part Number           Qty Item Desc                Vendor CLAAS Document ID
---------------       --- ---------                --------------------------
Reason: Part not found in Inventory.
2171570                10                          0000022019_201901301830
24069411                1                          0000022019_201901301903
```

*   Parts that were not ordered are listed at the bottom of that specific order, as well as the reason they were not shipped.
*   Parts that are not included in the order have a reference number listed on the same line to be used for reference if you need to contact CLAAS about the order.
...
```
AGR EQUIPMENT SALES                                        CLAAS Suggested Orders                           Date 2/14/19
Division                                                     Order Details                              Time 11:27:15

Requested Date: 1/23/19

Items Not Ordered:
Part Number           Qty Item Desc                Vendor CLAAS Document ID
---------------       --- ---------                --------------------------
Reason: Warehouse 0000022020 not configured.
0117992                 1                          0000022020_201901101443
0142400                 1                          0000022020_201901101446
0143554                 1                          0000022020_201901101446
...
```
...
The Relevant Items report is automatically generated and printed every Friday night on the default printer that you specify. The report lists parts in the Relevant Item file by CLAAS that are currently not in your division's inventory. This allows you to decide whether or not to add them.

*   If a part on a suggested stock order from CLAAS is not in the division's inventory, other divisions assigned to the CLAAS PIM, or the CLAAS Price Book, it is listed in the report with a message of **"No Data Found"**.
...
*   **Relevant Item File**: These files contain items that CLAAS believes should be in your dealership's item file and is requested by your system every Friday night from 8 PM until 5AM Saturday morning. If you do not run the job or the job fails while running, the last time the file was run successfully continues to be used.
...
tags: ["browser", "cache", "clear cache", "Google Chrome", "Mozilla Firefox", "Internet Explorer", "Safari", "troubleshooting", "web maintenance", "browser history"]
version: "1.0"
last_updated: "2024-05-24"
short_description: "A concise guide providing step-by-step instructions on how to clear the browser cache for major web browsers including Google Chrome, Mozilla Firefox, Internet Explorer, and Safari. The document explains why clearing the cache is important for ensuring up-to-date content and resolving display errors."
long_description: "This document, titled 'Clearing Your Browser Cache,' serves as a practical user guide for clearing stored browser data. It begins with an introduction explaining the purpose of a browser's cache—to speed up web browsing by storing local copies of website code and images. It then details how this cache can occasionally cause problems, such as displaying outdated information or causing pages to load incorrectly. The core of the document provides detailed, step-by-step instructions for clearing the cache on several popular web browsers: Google Chrome, Mozilla Firefox, Internet Explorer, and Safari. Each section is dedicated to a specific browser and includes keyboard shortcuts and screenshots to guide the user through the process. For example, it shows how to access the 'Clear Browsing Data' menu, select the appropriate data types to delete (like 'Cached images and files'), and choose the desired time range. The guide is designed to be easily followed by users of all technical levels to help them maintain their browsers and ensure a smooth, current web experience."