---
title: "Parts Physical Inventory Procedures with Group/Van Option"
source: "combined_Keyston.md"
tags: ["Keystone"]
version: "1.0"
last_updated: "2026-02-24"
short_description: "A document about Parts Physical Inventory Procedures with Group/Van Option."
long_description: "This document provides information about Parts Physical Inventory Procedures with Group/Van Option from the DIS Keystone system."
---

## INTRODUCTION

The following instructions cover the process of conducting physical inventory when you subscribe to the Group/Van option. These instructions are split into three sections to be completed before, during, and after taking inventory and should be completed in the order that they are listed. Once you have completed the instructions in a section, you do not need repeat the instructions until the next time that you conduct physical inventory.
...
1.  Select the On-Hand Quantity Report from the Keystone menu *(Parts | Parts Reports | On-Hand Quantity Report)* or *(Menu X36, option 5)*.
    ...
    - Select the **All** check box in the Group section.
    - Click **Enter**.
...
2.  Verify parts have bin locations. Select the Bin Location Range Report from the Keystone menu *(Parts | Parts Reports | Bin Location Range Report)* or *(Menu X36, option 14)*.
    ...
    - Leave the **From** and **To** fields in the Bin Location section blank.
    - Select the **Report all blank locations with Available Quantity** check box.
    - Click **Enter**.
    - If you find unassigned bin locations on the report that is generated, make note as they will be fixed in Step 5.
...
5.  Consider fixing bin locations that were entered incorrectly. For instructions on getting a DataMine of all bin numbers, **click here**. Contact DIS about replacing blank locations on bin vans if you want to run count sheets for all group/van locations. Confirm that all bin locations are valid and fix those that are not by completing the following steps:
    - Find any bins on the DataMine that were entered incorrectly, such as A2 vs. A-2.
...
6.  Find open counter tickets and work orders by selecting the Work-In-Process Report from the Keystone menu *(POS | POS Reports | Work In Process Report)* or *(Menu X56, option 5)*.
    ...
    - Leave the **Repair Orders** and **Counter Tickets** check boxes selected in the Print section.
...
1.  Select the Group Inventory Adjustment Report from the Keystone menu *(Parts | Parts Inventory Adjustment | Group Inventory Adjustment Report)* or *(Menu X33, option 7)*.
    On the screen that opens:
    - Select the **All Group Code** check box.
    - Select **Bin** for Sequence of report.
    - Select whether to **Include parts regardless of on-hand quantity** or **Include parts where on-hand quantity is 1 or more**.
...
| :--- | :--- |
| The report will sort and list bin locations from left to right by each character in the bin location. If you had the bin locations: AA1, AA2, AA11, AA22, AB1 – They would be listed in this order:<br><br>AA1<br>AA11<br>AA2<br>AA22<br>AB1 | The report will sort and list bin locations letters first, then numbers. Bin locations must begin with letters, end in numbers, and not mix. If you had the bin locations: AA1, AA2, AA11, AA22, AB1 – They would be listed in this order:<br><br>AA1<br>AA2<br>AA11<br>AA22<br>AB1 |
...
1.  Adjust part quantities by selecting Group Inventory Adjustment from the Keystone menu *(Parts | Parts Inventory Adjustment | Group Inventory Adjustment)* or *(Menu X33, option 6)*.
    On the screen that opens:
    - Type in the group number that you would like to adjust inventory for.
    ...
    - Click **Enter**.
    - The second Group Inventory Adjustment screen opens displaying the Group, Vendor, Part Number, and Description for the first part assigned to this bin.
    - Verify the quantity in the **On hand/Available** field.
      - If the quantity is correct, click **Enter**.
      - If the quantity is incorrect, click in the **On hand/Available** field and type in the correct quantity, then click **Enter**.
...
2.  Select the Group Inventory Exception Report from the Keystone menu *(Parts | Parts Inventory Adjustment | Group Inventory Exception Report)* or *(Menu X33, option 8)*.
    On the screen that opens:
    - The Date Selection field defaults to the most recent date for the "Last Count Date". Leave it as it is.
...
> **Summary of Group Inventory Exception Report:** This report prints a list of all parts that have a Last Count Date prior to the date entered on the Group Inventory Exception Report. The parts listed on this report are parts that are on the current count sheet whose quantities have not been confirmed in the Group Inventory Adjustment option. If every part number has been processed, there should be no parts on this report.
...
2.  Select Inventory Pad from the Keystone menu again *(Parts | Part Report | Parts Inventory Pad)* or *(Menu X36, option 1)*.
    On the screen that opens:
    - Leave the **All Vendors** and **All Classes** check boxes selected.
    - Click **Enter**. The second Parts Inventory Pad screen opens.
    - Unselect the **Print Detail** check box.
